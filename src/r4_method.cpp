...
R4Method::R4Method(float t, float beta, float nu, float lambda, float alpha){
    _t = t;
    _beta = beta;
    _nu = nu;
    _lambda = lambda;
...
float R4Method::f_s(std::map<type, float> args){
    return - this->_beta * args[S] * args[I] + this->_gamma * args[I] - this->_lambda * args[S];
}

float R4Method::f_i(std::map<type, float> args){
    return this->_beta * args[S] * args[I] - this->_gamma * args[I] - this->_nu * args[I] - this->_alpha * args[I];
}
